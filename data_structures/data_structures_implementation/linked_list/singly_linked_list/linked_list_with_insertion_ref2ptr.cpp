...
void append(Node*& head_ref, string data) {
  ...
  if (head_ref == nullptr) { // If not passing pointer-to-head or reference-to-head, this step would be a problem
    head_ref = new_node;
    return;
  }

  Node* last = head_ref; // last will point to the last node eventually
  while (last->next) {
    last = last->next;
  }
  last->next = new_node;
